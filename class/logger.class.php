...
class logger {
    ...
    private function type() {
		    if (preg_match("/issued server command:/", $this->log_)) {
				...
				$this->log_ = preg_replace("/\\[Server thread\\/INFO\\]:/", "<div style='color: #40ff00; padding: 0px 10px 0px 0px;'>[Server]</div>", $this->log_);
		    $this->log_ = preg_replace("/\\[Server thread\\/ERROR\\]:/", "<div style='color: #40ff00; padding: 0px 10px 0px 0px;'>[Server]</div>", $this->log_);
		    $this->log_ = preg_replace("/\\[Server thread\\/WARN\\]:/", "<div style='color: #40ff00; padding: 0px 10px 0px 0px;'>[Server]</div>", $this->log_);
